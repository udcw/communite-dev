...
export default function DashboardPage() {
  ...
  if (loading) {
    return <div className="text-center py-20">Chargement de votre tableau de bord...</div>;
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <span className="text-sm text-gray-500">Bienvenue {session?.user?.name} 👋</span>
      </div>
          ...
          <div className="space-y-3">
            {recentActivity.map((activity, index) => (
              <div key={index} className="flex items-center gap-3 text-sm border-b pb-2">
                ...
                <span className="text-gray-400 ml-auto">{new Date(activity.date).toLocaleDateString()}</span>
              </div>
            ))}
          </div>
        )}
